...
import com.whcd.base.component.easeui.controller.EaseUI.EaseSettingsProvider;
...
public class EaseNotifier {
	...
	protected final static String[] msg_eng = { "sent a message", "sent a picture", "sent a voice", "sent location message", "sent a video", "sent a file",
	...
	protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
		...
		try {
			...
			NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(appContext).setSmallIcon(appContext.getApplicationInfo().icon)
			...
			String summaryBody = msgs[6].replaceFirst("%1", Integer.toString(fromUsersNum)).replaceFirst("%2", Integer.toString(unreadMsgCountTotal));

			if (notificationInfoProvider != null) {
				// lastest text
				String customSummaryBody = notificationInfoProvider.getLatestText(message, fromUsersNum, unreadMsgCountTotal);
