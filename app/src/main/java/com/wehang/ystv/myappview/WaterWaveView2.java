...
public class WaterWaveView2 extends View {
    ...
    private String waterColor="#5061f25e";
    public void setWaveColor(String mWaveColor){
        this.waterColor = mWaveColor;
        init();
    }
    //初始化
    private void init() {
        progressPaint = new Paint();
        progressPaint.setColor(Color.parseColor(waterColor));
    ...
    protected void onDraw(Canvas canvas) {
       ...
       // canvas.drawCircle(centerX, centerY, VALID_RADIUS + (STROKE_WIDTH >> 1), circlePaint);
        //绘制水波曲线
        canvas.drawPath(getWavePath(xOffset*2), progressPaint);
        ...
        //测量文字高度
       // textPaint.getTextBounds("4", 0, 1, textBounds);
        float h1 = textBounds.height();
        float extraW = textPaint.measureText("4") / 3;
...
//        textPaint.getTextBounds("", 0, 1, textBounds);
        ...
        canvas.drawText("", centerX + w1 / 2 - extraW + 5, centerY - (h1 / 2 - h2), textPaint);
